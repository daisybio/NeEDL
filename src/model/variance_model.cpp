...
namespace epi {
    ...
    void
    VarianceModel<QuantitativePhenoType>::
    compute_test_statistic_(const std::vector<std::vector<QuantitativePhenoType>> & penetrance_table, double & test_statistic, std::size_t & num_groups) const {
        ...
        for (cell = penetrance_table.begin(), group_mean = group_means.begin(); cell != penetrance_table.end() and group_mean != group_means.end(); cell++, group_mean++) {
            if (cell->empty()) {
                continue;
            }
            ss_between_groups += static_cast<double>(cell->size()) * (*group_mean - global_mean_) * (*group_mean - global_mean_);
        ...
        if (ss_between_groups <= 0.0 or num_groups == 1) {
            ...
            test_statistic = (ss_between_groups / static_cast<double>(num_groups - 1)) / (ss_within_groups / static_cast<double>(this->instance_->num_inds() - num_groups));
        }
    }
    ...
    void
    VarianceModel<CategoricalPhenoType>::
    compute_test_statistic_(const std::vector<std::vector<CategoricalPhenoType>> & penetrance_table, double & test_statistic, std::size_t & num_groups) const {
        ...
        for (const auto & cell : penetrance_table) {
            ...
            for (CategoricalPhenoType phenotype{0}; static_cast<std::size_t>(phenotype) < this->instance_->num_categories(); phenotype++) {
                if (num_inds_in_categories_.at(phenotype) == 0) {
                    continue;
                }
                expected_cell_inds_in_category = static_cast<double>(num_inds_in_categories_.at(phenotype) * cell.size()) / static_cast<double>(this->instance_->num_inds());
                deviation_from_expectation = static_cast<double>(num_cell_inds_in_categories.at(phenotype)) - expected_cell_inds_in_category;
                test_statistic += (deviation_from_expectation * deviation_from_expectation) / expected_cell_inds_in_category;
            }
        }
    ...
    p_value_(double test_statistic, std::size_t num_groups) const {
        ...
        double return_value{1.0 - misc::f_cdf(test_statistic, num_groups - 1, this->instance_->num_inds() - num_groups)};
    ...
    p_value_(double test_statistic, std::size_t num_groups) const {
        ...
        double return_value{1.0 - misc::chi_squared_cdf(test_statistic, (num_groups - 1) * (num_non_empty_categories_ - 1))};
        return misc::ensure_valid_continuous_probability(return_value);
    }
}