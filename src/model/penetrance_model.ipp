...
namespace epi {
    ...
    evaluate_(const std::vector<SNP> & snp_set, bool prepare_prediction) {
        ...
        if (get_cov_status()) {
            ...
            // To incorporate covariates, we modify the calculation of the penetrance table.
            // We do this by fitting a linear regression model with the covariates as predictors and the phenotypes as the response variable.
            // We use the residuals from the regression as the observed phenotypes in the penetrance table.
        ...
        for (Ind ind{0}; ind < this->instance_->num_inds(); ind++) {
            ...
            if (get_cov_status()){
                penetrance_table.at(genotypes.back()).emplace_back(residuals(ind, 0));
            } else {
